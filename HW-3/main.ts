...
for (let user of users) {
    if (user.status) document.write(`<h2 style="color: red;">${user.name}</h2>`)
}

for (let user of users) {
    if (!user.status) document.write(`<h2 style="color: blue;">${user.name}</h2>`)
}

for (let user of users) {
    if (user.age > 30) document.write(`<h2 style="color: yellow;">${user.name}</h2>`)
}
