...
const calculateAreaRectangle1 = (a: number, b: number): number => {
    return a * b
}
...
const calculateAreaCircle1 = (r: number): number => {
    return PI1 * r * r
}
...
const calculateAreaCylinder1 = (r: number, h: number): number => {
...
const showItemArr1 = (arr: number[]): void => {
...
const writeMsg1 = (msg: string): void => {
...
//
const writeTag1 = (text: string): void => {
...
const writeList1 = (text: string, count: number): void => {
    document.write(`<ul>`);
...
const writeListOfArr1 = (arr: (string | number | boolean)[]): void => {
    document.write(`<ul>`);
...
const writeDiv1 = (arr: User5[]): void => {
    for (let arrElement of arr) {
        document.write(`<div>`);
        for (let objKey in arrElement) {
            document.write(`${objKey}: ${arrElement[objKey as keyof User5]}, `);
        }
        document.write(`</div>`);
    }
}
...
const findMin1 = (arr: number[]): number => {
    let min = arr[0];
...
const calculateSum1 = (arr: number[]): number => {
    let sum = 0;
...
const swap1 = (arr: number[], index1: number, index2: number): number[] => {
...
const exchange1 = (sumUAH: number, currencyValues: Value1[], exchangeCurrency: string): string | undefined => {
